...
#$dbh = DBI->connect("DBI:mysql:database=".$db.";host=localhost","jason","conell308",{'RaiseError' => 1});
...
$prepFull = $dbh->prepare("select distinct comment,label,source,destination,multTag,consumer,spsList from ".$db.".".$table." where label is not null order by label + 0 asc;");
$prepFull->execute();
...
sub provider {
	...
	if ( $table eq "ddnReqLabels" || $table eq "ddnPublishers" ) {
		...
		$prepFull = $dbh->prepare("select distinct provider,type,SrcID,constituents,cvaMultTag,sps from ".$db.".".$table." where label = $label;");

	}

	else {	

		$prepFull = $dbh->prepare("select distinct provider,type,SrcID,constituents,sps from ".$db.".".$table." where label = $label;");
...
sub test {
	...
	$prepFull = $dbh->prepare("select distinct instSide,instID from ".$db.".".$table." where label = \"$label\" and provider = \"$prov\";");
	$prepFull->execute();
